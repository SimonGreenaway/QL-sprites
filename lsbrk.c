...
//char *SJG_mheap = (char *)NULL;        /* Pointer to level 1 maintained Heap Areas */
//char *SJG_mbase = (char *)NULL;        /* Free memory list */
//long  SJG_mfree = 0L;                  /* Amount of memory in free memory list */
//long  SJG_mtotal = 0L;                 /* Total memory allocated so far */

//SJG:
extern char *_mheap,*_mbase;
extern long _mfree,_mtotal;
...
static void *SJGsys_lsbrk _LIB_F1_ (long,   lnbytes )
{
    char *ret;
    long  newlen, maxlen, lenbytes;

    //printf("Trying to malloc %ld\n",lnbytes);
	...
	printf("Alloced %ld at %d\n",lnbytes,(unsigned int)ret);
	debugMalloc();

        return ((void *)ret);
    }

    printf("Need more memory! (%ld)\n",_memincr);
    /*
...
void *SJGlsbrk _LIB_F1_ (long,   lnbytes )
{
    ...
    if (SYSCALL1(0,&sctl,SJGsys_lsbrk,lnbytes) < 0)
    {
        return SJGsys_lsbrk(lnbytes);
    }
